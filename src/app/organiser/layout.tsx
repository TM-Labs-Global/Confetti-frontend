...
const NAV = [
  ...
  { label: 'Plan an Event', path: '/organiser/create-plan' },
...
export default function OrganizerLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    else if (user.role !== 'organiser') router.replace(`/${user.role}/dashboard`)
  }, [mounted, loading, user, router])

  if (!mounted || loading || !user || user.role !== 'organiser') return null
    ...
    <div className="flex h-screen bg-canvas">
      <aside className="w-[248px] bg-dark flex flex-col shrink-0 border-r border-dark-border">
        ...
        <nav className="flex-1 px-4">
          ...
          {NAV.map(({ label, path }) => {
            const isActive = pathname === path || (path !== '/organiser/dashboard' && pathname?.startsWith(path))
            return (
              <Link
                key={path}
                href={path}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[57px] bg-white border-b border-border flex items-center px-8 shrink-0 z-10">
          <span className="font-display font-semibold text-[14px] text-ink-2">Organiser Portal</span>
