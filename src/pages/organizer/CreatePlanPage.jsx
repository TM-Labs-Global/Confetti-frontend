...
function WizardProgress({ step, total, onBack }) {
  return (
    <div className="mb-10">
...
function Step1({ form, onChange, eventTypes }) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {eventTypes.map(type => {
          const meta = EVENT_META[type.id] ?? { emoji: '🎉', bg: '#F5F5F5', tagline: '' }
          ...
          return (
            ...
            >
              <span className="text-[26px] block mb-2">{meta.emoji}</span>
        ...
        <div className="bg-white border border-border rounded-xl p-6 space-y-5">
          ...
          <div>
            ...
            {Number(form.totalBudget) > 0 && (
              <p className="text-[12px] text-ink-3 mt-1.5">{fmtNaira(form.totalBudget)} total event budget</p>
            )}
          </div>
        </div>
      )}
...
function Step2({ form, onChange, getCatsForType }) {
...
function Step3({ form, onChange, getCatsForType }) {
  const presetCats = getCatsForType(form.eventType)
  ...
  function applyMode(mode) {
    ...
    if (mode === 'even') {
      ...
      const a = {}; allCats.forEach(c => { a[c.id] = per }); onChange('allocations', a)
    } else if (mode === 'smart') {
      const a = {}
  ...
  function setAmt(id, val) { onChange('allocations', { ...form.allocations, [id]: val }) }
...
function Step4({ form, getCatsForType, onPublish, onSave, saving }) {
  const presetCats   = getCatsForType(form.eventType)
  const allCats      = [
    ...presetCats.filter(c => form.selectedCategories.includes(c.id)),
    ...form.customCategories.filter(c => form.selectedCategories.includes(c.id)),
  ]
  const meta        = EVENT_META[form.eventType] ?? { emoji: '🎉', bg: '#F5F5F5' }
  const totalBudget = Number(form.totalBudget) || 0
    ...
    <div>
      ...
      <div className="bg-white border border-border rounded-xl p-6 mb-4">
        <div className="flex items-start gap-4">
          <span className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0" style={{ background: meta.bg }}>{meta.emoji}</span>
          <div className="flex-1 min-w-0">
...
export default function CreatePlanPage() {
  ...
  function getCatsForType(typeId) {
