...
export default function OrganizerDashboard() {
  ...
  const activePlans = plans.filter(p => ['open', 'bidding', 'in-progress'].includes(p.status))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Open for bids"  value={plans.filter(p => p.status === 'open').length} />
        <StatCard label="In progress"    value={plans.filter(p => p.status === 'in-progress').length} />
      </div>
          ...
          <div className="divide-y divide-border">
            {recentPlans.map(plan => {
              ...
              return (
                ...
                >
                  <span className="w-9 h-9 rounded-lg flex items-center justify-center text-lg shrink-0"
