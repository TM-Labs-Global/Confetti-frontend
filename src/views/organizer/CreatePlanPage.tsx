...
interface FormState {
  ...
  // Also broadcast this budget to the open vendor marketplace (opt-in). Off = the
  // organiser finds and invites vendors themselves; on = vendors can bid too.
  openToBids: boolean
}
...
const EMPTY_FORM: FormState = {
  ...
  lineItems: {}, briefs: {}, allocationMode: 'recommended', openToBids: false,
}
...
function Step4({ form, onChange, getCatsForType, onPublish, onSave, onBack, saving, isEdit, primaryLabel }: Step4Props) {
  const presetCats   = getCatsForType(form.eventType)
  const allCats      = [
    ...presetCats.filter(c => form.selectedCategories.includes(c.id)),
    ...form.customCategories.filter(c => form.selectedCategories.includes(c.id)),
  ...
  return (
    <div>
      ...
      <label className="flex items-start gap-3 mb-4 rounded-xl border border-border bg-white px-4 py-3.5 cursor-pointer hover:border-primary/40 transition-colors">
        <input type="checkbox" checked={form.openToBids}
          onChange={e => onChange('openToBids', e.target.checked)}
          className="mt-0.5 w-4 h-4 rounded accent-primary shrink-0" />
        <span className="min-w-0">
          <span className="block text-[13px] font-medium text-ink">Also let vendors find this budget and send bids</span>
          <span className="block text-[12px] text-ink-3 mt-0.5">
            We&apos;ll list it in the open marketplace so any verified vendor can bid. Leave this off to keep it private and invite vendors yourself. You can change this anytime.
          </span>
        </span>
      </label>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <button onClick={onPublish} disabled={!!saving}
          ...
          {saving === 'open' ? 'Saving…' : primaryLabel}
        </button>
      </div>
    </div>
  )
...
export default function CreatePlanPage({ surface = 'organiser' }: { surface?: WizardSurface } = {}) {
  ...
  async function submit(status: 'open' | 'draft') {
    ...
    try {
      ...
      } else if (status === 'open' && newId) {
        // New organiser event: go straight to Find Vendors, scoped to this event,
        // so they can invite vendors (and/or watch bids roll in) right away.
        router.push(`/organiser/marketplace?plan=${newId}`)
      } else {
        router.push('/organiser/plans')
      }
    } catch {
      submittingRef.current = false
      setSaving(null)
    ...
    <div className="max-w-[680px] mx-auto">
      ...
      {step === 4 && (
        <Step4
          ...
          primaryLabel={
            surface === 'admin'
              ? (isEdit ? 'Save & Publish' : 'Publish event')
              : (isEdit ? 'Save & Publish' : 'Find Vendors')
          }
        />
      )}
